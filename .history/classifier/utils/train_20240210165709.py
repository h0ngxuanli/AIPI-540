...
def train_model(run, model, model_name, dataloaders, criterion, optimizer, scheduler, device, epochs):
    ...
    for epoch in tqdm(epochs, desc='Processing'):
        ...
        for phase in ['train', 'val']:
            ...
            y_trues = torch.zeros((len(dataloaders[phase].dataset), 1), dtype=torch.float32, device=device)
            y_preds = torch.zeros((len(dataloaders[phase].dataset), 1), dtype=torch.float32, device=device)
            ...
            print('Epoch {:03}: | Loss: {:.3f} | {} acc: {:.3f} | Val acc: {:.3f} | Val roc_auc: {:.3f} | Training time: {}'.format(
                        epoch + 1, 
                        epoch_loss, 
                        train_acc, 
                        val_acc, 
            ...
            if phase == 'val' and epoch_auc > best_auc:
                ...
                best_model_path = Path("./model/checkpoints") / (model_name)
