...
def train_model(run, model, model_name, dataloaders, criterion, optimizer, scheduler, device, epochs):
    ...
    for epoch in tqdm(epochs, desc='Processing'):
        ...
        for phase in ['train', 'val']:
            ...
            true_labels = torch.zeros((len(dataloaders[phase].dataset), 1), dtype=torch.float32, device=device)
            ...
            for i, (inputs, labels) in enumerate(dataloaders[phase]):
                ...
                true_labels[i*dataloaders[phase].batch_size:(i*dataloaders[phase].batch_size + inputs.size(0))]
